...
- **個資洩漏 72h 通報（NFR-SEC-001）**：依台灣個資法 Art.12，個資外洩事件須於知悉後 72 小時內通報主管機關（國家發展委員會）；系統須具備事件日誌與通報流程文件（見 RUNBOOK.md §Security Incident）
- **GDPR 適用邊界控制（NFR-SEC-002）**：ToS 須明確禁止歐盟用戶使用（降低 GDPR 適用風險）；若偵測到 EU IP 存取，顯示地區限制提示；非本版強制實作，由 Legal 確認後納入 v2.0
