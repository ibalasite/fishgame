...
sequenceDiagram
    ...
    participant Ingress as Nginx Ingress<br/>:443
    participant CR as GameController<br/>(Colyseus Room)
    participant SUC as ShootFishUseCase
    participant RTP as RTPService<br/>(RTPEngine)
    participant JP as JackpotApplicationService
    participant Redis as Redis<br/>:6379
    participant MySQL as MySQL<br/>:3306
    participant EB as DomainEventBus<br/>(Redis Pub/Sub)
    ...
    CR->>CR: validateJWT(token)<br/>check player is in room
    ...
    RTP->>RTP: adjustHitProbability(currentRTP=0.88)<br/>targetRTP=0.90 → increase hit rate
    RTP->>RTP: rollDice() — isHit = true

    Note over RTP,SUC: Phase 3 — 命中確認 + 扣血
    RTP-->>-SUC: HitResult{isHit: true, coinsAwarded: 500}
    SUC->>SUC: fish.applyDamage(damage=120)<br/>fish.hp: 300 → 180
    ...
    SUC->>+MySQL: INSERT fish_kills<br/>{sessionId, killerId, fishType, coins=500, rtp=0.88}
    MySQL-->>-SUC: OK (id=kill_001)
    SUC->>+Redis: SET rtp_state:{playerId}<br/>{currentRTP: 0.882, sessionShots: 43}
    Redis-->>-SUC: OK

    Note over SUC,EB: Phase 6 — 發布領域事件（異步）
    SUC->>+EB: publish FishKilled<br/>{sessionId, killerId, fishType, coins=500}
    ...
    CR->>CR: updateRoomState()<br/>fish.hp=180, player.coins+=500
    ...
    CC->>CC: renderHitAnimation("NORMAL_HIT")<br/>updateCoinDisplay(85500)

    Note over EB,Shop: Phase 8 — 異步結算（Commerce BC）
    EB->>+Shop: consume FishKilled event<br/>{killerId, coins=500}
    Shop->>+MySQL: UPDATE users SET gold_balance += 500<br/>WHERE id = killerId
