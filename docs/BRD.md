...
| O4 | 驗證玩家留存健康度 | 次日留存率 / 7 日留存率 | N/A | 次日 ≥ 35%，7 日 ≥ 25% | MVP 上線後 2 週 | Should |
...
| 7 日留存率 ≥ 25% | N/A | 25% | Launch + 2W | 分析後台（同期群分析） | PM + 數值策劃 | 緊急用戶訪談，調整 RTP 數值或首週引導流程 |
| 次日留存率 ≥ 35% | N/A | 35% | Launch + 1W | 分析後台 | PM + 數值策劃 | 回顧新手引導與首局體驗，啟動快速迭代 |
...
| 角色 | 姓名 | 簽核狀態 | 日期 | 備注 |
|------|------|:-------:|:----:|------|
| Executive Sponsor（投資方/創辦人）| 待確認 | 🔲 待簽核 | — | 核准後方可啟動 PRD 撰寫 |
| Product Lead / PM | 待確認 | 🔲 待簽核 | — | |
| Engineering Lead | 待確認 | 🔲 待簽核 | — | 簽核前需完成 PoC 評估意見 |
| Finance（投資方財務代表）| 待確認 | 🔲 待簽核 | — | 三情境 ROI 審查確認 |
| Legal / Compliance | 待確認 | 🔲 待簽核 | — | **阻擋性條件**：台灣+東南亞法規初審結果出爐前不可核准 |
