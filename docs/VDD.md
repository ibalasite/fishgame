...
| 類型 | 背景 | 邊框 | 圓角 | 陰影 |
|------|------|------|------|------|
| 標準面板 | `var(--card-bg)` (rgba(10,35,64,0.95)) | 1px solid `var(--card-border)` (= rgba(245,200,66,0.55)) | `--radius-lg`（24px） | `var(--card-shadow)` |
| 高亮面板（VIP/特殊） | `var(--card-bg-highlight)` | 2px solid `var(--color-border-brand)` (#F5C842) | `--radius-lg`（24px） | `var(--card-shadow-highlight)` |
| 模態背景遮罩 | `var(--modal-overlay)` (rgba(5,20,40,0.85)) backdrop-blur 8px | — | — | — |
| 武器選擇卡 | `var(--weapon-card-bg)` (rgba(13,51,96,0.9)) | 2px solid `var(--weapon-card-border)` | `--radius-md`（16px） | `var(--weapon-card-shadow)` |
| 商品卡片 | `var(--shop-card-bg)` (rgba(15,10,0,0.9)) | 1px solid `var(--shop-card-border)` | `--radius-lg`（24px） | `var(--shop-card-shadow)` |
