...
| Hover | `var(--btn-secondary-bg-hover)` (rgba(0,212,255,0.1)) | `color-action-secondary-hover`（#40DFFF） | 2px solid `color-action-secondary-hover`（#40DFFF） |
| Active | `var(--btn-secondary-bg-active)` (rgba(0,212,255,0.2)) | `color-action-secondary`（#00D4FF） | 2px solid `color-action-secondary-active`（#00A8CC） |
...
| 高亮面板（VIP/特殊） | rgba(10,35,64,0.98) | 2px solid `var(--color-border-brand)` (#F5C842) | `--radius-lg`（24px） | 0 0 24px rgba(245,200,66,0.4) |
...
| 武器冷卻圓圈 | 炮台右側 | 48×48 px | 底 rgba(0,0,0,0.5)，進度弧 `var(--color-action-secondary)` | text-caption 12px（中央倒數） |
...
--shop-card-border:        rgba(245,200,66,0.5);  /* color-gold-400 @ 50%；WCAG 1.4.11 ≥3:1（3.4:1 on #0F0A00）；與 weapon-card-border 對齊；無匹配 primitive，直接值 */
...
| 背景 | 深海漸層 `var(--color-bg-base)`→#030B1A，帶輕微放射光（中心 `var(--color-bg-surface)`） |
