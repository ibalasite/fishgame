...
| 狀態 | 背景 | 文字色 | 邊框 | 陰影 | 效果 |
|------|------|-------|------|------|------|
| Default | `var(--btn-primary-bg)` (#F5C842 金屬漸層→#C99A00) | `var(--btn-primary-text)` #0A1A00 | 無 | `var(--btn-primary-shadow)` | — |
| Hover | `var(--btn-primary-bg-hover)` (#C99A00) | `var(--btn-primary-text)` #0A1A00 | 無 | `var(--btn-primary-shadow)` | scale(1.03) |
| Active / Press | `var(--btn-primary-bg-active)` (#C99A00) | `var(--btn-primary-text)` #0A1A00 | 無 | `var(--shadow-inset)` | scale(0.98), translateY(2px) |
| Focus | `var(--btn-primary-bg)` (#F5C842) | `var(--btn-primary-text)` #0A1A00 | 3px solid `var(--btn-primary-focus-ring)` + 2px outline `var(--color-border-focus)` | — | Ring 11.63:1 AAA |
| Disabled | `var(--btn-disabled-bg)` (#0A2340) | `var(--btn-disabled-text)` (#5C6673) | 無 | 無 | opacity 0.5；cursor: not-allowed |

**尺寸**：高度 56 px，水平 Padding 24 px，圓角 `var(--radius-md)`（16 px），最小寬度 160 px；字型：`text-button-label`（18 px / 22 px / 500）

#### 次要按鈕（Secondary Button）

| 狀態 | 背景 | 文字色 | 邊框 |
|------|------|-------|------|
| Default | transparent | `var(--color-action-secondary)`（#00D4FF） | 2px solid `var(--color-action-secondary)`（#00D4FF） |
| Hover | `var(--btn-secondary-bg-hover)` (= rgba(0,212,255,0.1)) | `var(--color-action-secondary-hover)`（#40DFFF） | 2px solid `var(--color-action-secondary-hover)`（#40DFFF） |
| Active | `var(--btn-secondary-bg-active)` (= rgba(0,212,255,0.2)) | `var(--color-action-secondary)`（#00D4FF） | 2px solid `var(--color-action-secondary-active)`（#00A8CC） |
| Focus | transparent | `var(--color-action-secondary)`（#00D4FF） | 3px solid `var(--btn-primary-focus-ring)` + 2px outline `var(--color-action-secondary)`（#00D4FF） |
| Disabled | transparent opacity 0.4 | `var(--btn-secondary-disabled-color)` | 2px solid `var(--btn-secondary-disabled-color)` |

**尺寸**：高度 48 px，水平 Padding 20 px，圓角 `var(--radius-md)`（16 px）；字型：`text-button-label`（18 px / 22 px / 500）

#### 圖示按鈕（Icon Button）

| 狀態 | 圖示色 | 背景 | 效果 |
|------|-------|------|------|
| Default | `var(--color-text-secondary)` (#9BA1A9) | transparent | — |
...
**尺寸**：40×40 px，圓角 `var(--radius-icon)`（20 px，圓形），觸控熱區 ≥ 44×44 px
...
| Active | `var(--color-red-700)` = #CC0000 | #FFFFFF | 無 | scale(0.98)，`var(--shadow-inset)` |
| Focus | `var(--color-action-danger)` (#FF4444) | #FFFFFF | 3px solid `var(--btn-primary-focus-ring)` + 2px outline `var(--color-action-danger)` | Ring 4.6:1 AA |
| Disabled | `var(--color-action-danger)` opacity 0.5 | `var(--color-white-40)` | 無 | cursor: not-allowed |

**尺寸**：高度 48 px，水平 Padding 20 px，圓角 `var(--radius-md)`（16px）；字型：`text-button-label`（18 px / 22 px / 500）

### 5.4 輸入欄位規格

| 狀態 | 背景 | 邊框 | 文字色 | 提示文字色 |
|------|------|------|-------|----------|
| Default | `var(--input-bg)` (rgba(255,255,255,0.05)) | 1px solid `var(--color-border-default)`（rgba(255,255,255,0.45)） | `var(--color-text-primary)` | `var(--input-placeholder)` (rgba(255,255,255,0.4)) |
| Hover | `var(--input-bg-hover)` (rgba(255,255,255,0.07)) | 1px solid `var(--color-border-strong)`（rgba(255,255,255,0.7)） | `var(--color-text-primary)` | `var(--input-placeholder)` |
| Focus | `var(--input-bg-focus)` (rgba(255,255,255,0.08)) | 2px solid `var(--color-border-focus)`（#F5C842）| `var(--color-text-primary)` | — |
| Error | `var(--input-bg-error)` | 2px solid `var(--color-border-error)`（#FF4444） | `var(--color-text-primary)` | — |
| Disabled | `var(--input-bg-disabled)` | 1px solid `var(--color-border-subtle)`（rgba(255,255,255,0.2)） | `var(--color-text-disabled)`（rgba(255,255,255,0.35)） | — |

**尺寸**：高度 52 px，水平 Padding 16 px，圓角 `var(--radius-md)`（16 px）

### 5.5 卡片 / 面板規格

| 類型 | 背景 | 邊框 | 圓角 | 陰影 |
|------|------|------|------|------|
| 標準面板 | `var(--card-bg)` (rgba(10,35,64,0.95)) | 1px solid `var(--card-border)` (= rgba(245,200,66,0.55)) | `var(--radius-lg)`（24px） | `var(--card-shadow)` |
| 高亮面板（VIP/特殊） | `var(--card-bg-highlight)` | 2px solid `var(--color-border-brand)` (#F5C842) | `var(--radius-lg)`（24px） | `var(--card-shadow-highlight)` |
| 模態背景遮罩 | `var(--modal-overlay)` (rgba(5,20,40,0.85)) backdrop-blur 8px | — | — | — |
| 武器選擇卡 | `var(--weapon-card-bg)` (rgba(13,51,96,0.9)) | 2px solid `var(--weapon-card-border)` | `var(--radius-md)`（16px） | `var(--weapon-card-shadow)` |
| 商品卡片 | `var(--shop-card-bg)` (rgba(15,10,0,0.9)) | 1px solid `var(--shop-card-border)` | `var(--radius-lg)`（24px） | `var(--shop-card-shadow)` |

### 5.6 HUD 元素規格

| 元素 | 位置 | 尺寸 | 顏色 | 字級 |
|------|------|------|------|------|
| 玩家分數 | 各角落（1P 左下，2P 右下，3P 左上，4P 右上） | 120×40 px | bg `var(--hud-bg)`，文字 `var(--hud-text-color)` | text-hud-score 28px |
| 金幣計數器 | 分數框右側 | 80×32 px | 圖示 `var(--hud-coin-color)`，數字 `var(--hud-text-color)` | text-hud-counter 20px |
| 鑽石計數器 | 金幣右側 | 80×32 px | 圖示 `var(--hud-diamond-color)`，數字 `var(--hud-text-color)` | text-hud-counter 20px |
| Jackpot 進度條 | 頂部居中 | 360×24 px | 底 `var(--color-bg-surface)`，填充 `var(--jackpot-bar-fill-start)`→`var(--jackpot-bar-fill-end)` 漸層 | — |
| Boss HP 條 | 頂部（Boss 進場時出現） | 480×32 px | 邊框 `var(--boss-hp-border)`，HP 填充 `var(--boss-hp-fill-high)`→`var(--boss-hp-fill-low)` | text-caption 12px |
| 武器冷卻圓圈 | 炮台右側 | 48×48 px | 底 `var(--weapon-cooldown-bg)`，進度弧 `var(--color-action-secondary)` | text-caption 12px（中央倒數） |
| 計時器 | 頂部左側 | 80×32 px | bg 透明，文字 `var(--hud-text-color)` | text-hud-counter，font-mono |
| 場次編號 | 頂部右側 | 60×20 px | 文字 `var(--color-text-secondary)` (rgba(255,255,255,0.6)) | text-caption |
...
|------|------|
| 高度 | 72 px（含安全區） |
| 背景 | `var(--navbar-bg)` backdrop-blur 12px |
| 頂部邊框 | 1px solid `var(--navbar-border-top)` |
| 圖示尺寸 | 28×28 px |
| 標籤字級 | text-caption 12px |
| 選中狀態 | 圖示 `var(--color-action-primary)`，標籤 `var(--color-action-primary)`，底部 3px `var(--color-action-primary)` 高亮條 |
| 未選中狀態 | 圖示 `var(--navbar-inactive-color)`，標籤 `var(--navbar-inactive-color)` |
...
| 狀態 | 背景 | 邊框 | 文字色 | 說明 |
|------|------|------|-------|------|
| Default | `var(--color-bg-surface)`（#0A2340） | 1px solid `var(--color-border-default)`（rgba(255,255,255,0.45)） | `var(--color-text-primary)`（#FFFFFF） | 預設收起狀態 |
| Hover | `var(--color-bg-elevated)`（#0D3360） | 1px solid `var(--color-border-focus)`（#F5C842） | `var(--color-text-primary)` | 滑鼠懸停 / 觸控長押前 |
| Focus | `var(--color-bg-surface)`（#0A2340） | 2px solid `var(--color-border-focus)`（#F5C842） + 2px outline `var(--color-ocean-900)`（#051428）| `var(--color-text-primary)` | 鍵盤 Tab 焦點；focus ring 外層深色確保可見（11.63:1 AAA）|
| Open（展開） | `var(--color-bg-elevated)`（#0D3360） | 2px solid `var(--color-border-focus)`（#F5C842） | `var(--color-text-primary)` | Dropdown list 顯示；list container 使用 `var(--shadow-card)` |
| Selected（選中項） | `var(--color-action-primary)`（#F5C842） | 無 | `var(--color-bg-base)`（#051428，深色反白） | List item 已選中行，高亮區分 |
| Disabled | `var(--color-bg-surface)` opacity 40% | 1px solid `var(--color-border-default)` opacity 40% | `var(--color-text-disabled)` | 不可交互；cursor: not-allowed；pointer-events: none |

**尺寸規格**：高度 48 px，水平 Padding 16 px，圓角 `var(--radius-md)`（16 px）；Dropdown list 最大高度 240 px（超出時啟用 ScrollView）
...
--color-player-orange: #FF6D00; /* 玩家3 識別色 / 房間滿員 badge / 暢銷 badge */
--color-player-purple: #9C27B0; /* 玩家4 識別色 / 深海女皇 Boss 主色 */
...
--color-white-8:   rgba(255,255,255,0.08); /* white @ 8%；btn icon hover / input focus 共用 */
...
--color-black-50:  rgba(0,0,0,0.5);  /* weapon-cooldown-bg */
--color-black-900: #000000;
--color-gold-border-50: rgba(245,200,66,0.5);  /* color-gold-400 @ 50%；weapon-card/shop-card border */
...
--shadow-glow-gold-soft: 0 0 24px rgba(245,200,66,0.4);  /* card-highlight glow，softer variant */
...
--shadow-card-sm:        0 4px 16px rgba(0,0,0,0.5);     /* shop-card shadow */
--shadow-inset:          inset 0 2px 4px rgba(0,0,0,0.4);
--shadow-inset-highlight: inset 0 1px 0 rgba(255,255,255,0.1);  /* weapon-card inner highlight */
...
--color-bg-overlay:     rgba(5,20,40,0.85);        /* = color-ocean-900 (#051428) @ 85% opacity；無匹配 primitive，直接值 */
...
--color-text-inverse:   #0A1A00;                             /* on 金色底；12.3:1 AAA（§9.1）；深森林綠，on 金色底專用；無匹配 primitive，直接值 */
...
--color-gold-border-subtle: rgba(245,200,66,0.55);  /* color-gold-400 @ 55%；WCAG 1.4.11 UI boundary ≥3:1（~4.2:1 on ocean-900）；Cocos 環境不支援 CSS color-mix，無法生成 opacity primitive，無匹配 primitive，直接值；HUD/卡片玻璃面板邊框主題色（多個元件共用）*/
--color-gold-border-card:  var(--color-gold-border-50);  /* rgba(245,200,66,0.5)；武器卡/商品卡邊框（稍暗，有意與 HUD/卡片 0.55 區分層次）*/
...
--btn-primary-px:          24px;
--btn-disabled-bg:         var(--color-bg-surface);    /* #0A2340 = color-ocean-800；所有按鈕禁用狀態共用底色 */
--btn-disabled-text:       var(--color-text-disabled); /* composited #5C6673；WCAG 1.4.3 豁免（禁用元件）*/
...
--btn-secondary-bg-active:     rgba(0,212,255,0.2);   /* neon-blue @ 20%，無匹配 primitive，直接值 */
--btn-secondary-disabled-color: rgba(0,212,255,0.4);  /* neon-blue @ 40%，無匹配 primitive，直接值 */
--btn-secondary-radius:        var(--radius-md);
--btn-secondary-height:        48px;

/* ── Button — Icon ── */
--icon-btn-bg-hover:  var(--color-white-8);    /* white @ 8%；via --color-white-8 */
--icon-btn-bg-active: rgba(255,255,255,0.12);  /* white @ 12%，無匹配 primitive，直接值 */
...
--card-bg-highlight:       rgba(10,35,64,0.98);   /* color-ocean-800 @ 98%（VIP/特殊高亮面板），無匹配 primitive，直接值 */
--card-border:             var(--color-gold-border-subtle);   /* color-gold-400 @ 55%；via --color-gold-border-subtle */
--card-border-width:       1px;
--card-radius:             var(--radius-lg);
--card-shadow:             var(--shadow-card);
--card-shadow-highlight:   var(--shadow-glow-gold-soft);      /* 0 0 24px rgba(245,200,66,0.4)；via --shadow-glow-gold-soft */

/* ── Input ── */
--input-bg:                rgba(255,255,255,0.05);  /* white @ 5%，無匹配 primitive，直接值 */
--input-bg-hover:          rgba(255,255,255,0.07);  /* white @ 7%，無匹配 primitive，直接值 */
--input-bg-focus:          var(--color-white-8);    /* white @ 8%；via --color-white-8 */
--input-bg-error:          rgba(255,68,68,0.08);    /* red-500 @ 8%，無匹配 primitive，直接值 */
--input-bg-disabled:       rgba(255,255,255,0.02);  /* white @ 2%，無匹配 primitive，直接值 */
...
--age-gate-overlay:        rgba(5,20,40,0.92);       /* 法規合規遮罩，比標準模態（0.85）更不透明；無匹配 primitive，直接值 */
...
--weapon-card-border:      var(--color-gold-border-card);  /* rgba(245,200,66,0.5)；via §6.2 --color-gold-border-card；武器卡稍暗金色（與 HUD/卡片 0.55 有意區分）*/
--weapon-card-shadow:      var(--shadow-inset-highlight);  /* inset 0 1px 0 rgba(255,255,255,0.1)；via --shadow-inset-highlight */
--weapon-card-radius:      var(--radius-md);
--weapon-card-selected-glow: var(--shadow-glow-gold);

/* ── Shop Card ── */
--shop-card-bg:            rgba(15,10,0,0.9);     /* 商店場景暖黑 @ 90%，無匹配 primitive，直接值 */
--shop-card-border:        var(--color-gold-border-card);   /* rgba(245,200,66,0.5)；via §6.2 --color-gold-border-card；WCAG 1.4.11 ≥3:1（3.4:1 on #0F0A00）*/
--shop-card-shadow:        var(--shadow-card-sm);           /* 0 4px 16px rgba(0,0,0,0.5)；via --shadow-card-sm */

/* ── Weapon UI ── */
--weapon-cooldown-bg:      var(--color-black-50);  /* rgba(0,0,0,0.5)；via --color-black-50；武器冷卻圓圈底板 */

/* ── NavBar ── */
--navbar-bg:               rgba(5,20,40,0.96);     /* color-ocean-900 @ 96%，無匹配 primitive，直接值 */
--navbar-border-top:       rgba(245,200,66,0.2);   /* color-gold-400 @ 20%，無匹配 primitive，直接值 */
--navbar-inactive-color:   rgba(255,255,255,0.5);  /* white @ 50%；介於 color-white-45/color-white-60，無精確 primitive，直接值 */
...
| 背景 | 深海漸層 `var(--color-bg-base)`→`var(--color-ocean-950)`，帶輕微放射光（中心 `var(--color-bg-surface)`） |
...
| 副標題 | Y=380，16 px，`var(--color-white-70)`，「最直接的競技捕魚體驗」 |
...
| 年齡聲明 | Y=900，text-caption，「點擊登入表示您已年滿 18 歲」，rgba(255,255,255,0.5)（無精確 primitive，介於 white-40/white-60 之間） |
...
| 元素 | 規格 |
|------|------|
| 遮罩 | full-screen，`var(--age-gate-overlay)`（rgba(5,20,40,0.92)），backdrop-blur 8px；比標準模態遮罩（0.85）更不透明，法規合規要求強制阻擋場景 |
| 面板 | 600×400 px，居中，card-bg 規格，`var(--color-border-brand)` 金色邊框 2px |
| 標題圖示 | 警示盾牌圖示 64×64 px，`var(--color-action-primary)` |
| 標題文字 | text-h1，「年齡確認」|
...
| 版本號 | 面板右下角，text-caption，rgba(255,255,255,0.3)（無精確 primitive，介於 white-20/white-35 之間） |
...
| 元素 | 規格 |
|------|------|
| Header Bar | 高度 80 px，bg rgba(5,20,40,0.96)（`var(--color-bg-base)` @ 96%，無精確 primitive），Logo 左側 120×40 px，右側金幣+鑽石顯示（§5.6 HUD 規格）+ 頭像 40×40 px 圓形 |
...
| 房間滿人 | 卡片 overlay 50% 暗化，「房間已滿」badge，橘色 `var(--color-player-orange)` |
...
| 元素 | 規格 |
|------|------|
| 背景 | 旋轉海底探照燈光效，`var(--color-bg-base)` 底色 |
| 標題 | text-h1，「配對中...」，Y=200 |
| 玩家位置（4格） | 2×2 格，各 160×200 px，已配對顯示頭像+名字，未配對顯示 Skeleton（骨架屏） |
| 配對進度 | 進度文字「2/4 玩家已加入」，text-body，`var(--color-action-secondary)` |
| 等待計時 | 計時器 MM:SS，text-hud-score，`var(--color-text-primary)` |
...
| 2P 炮台 | 右下角 Y=1120 | 同上，顏色不同（2P 青色 tint `var(--color-action-secondary)`） |
| 3P 炮台 | 左上角 Y=20 | 180° 翻轉，顏色 `var(--color-player-orange)` |
| 4P 炮台 | 右上角 Y=20 | 同 3P，顏色 `var(--color-player-purple)` |
...
| 特效 | 觸發 | 持續時間 | 粒子數量 | 顏色 |
|------|------|---------|---------|------|
| 炮彈軌跡 | 每次發射 | 100ms | 8–12 粒（拖尾） | `var(--color-action-primary)`→透明 |
| 普通魚死亡 | 擊殺 | 300ms | 10–20 金幣粒子 | `var(--color-action-primary)` |
| 精英魚死亡 | 擊殺 | 500ms | 30–50 金幣+光芒 | `var(--color-action-primary)` + `var(--color-text-primary)` |
| Boss 受擊 | 每次命中 | 150ms | 5–8 衝擊粒子 | `var(--color-feedback-error)` |
| Boss 死亡 / Jackpot | 觸發 | ≥3000ms | 200+ 金幣+光柱 | `var(--color-action-primary)` + `var(--color-vip-gold)` + `var(--color-text-primary)` |
| 技能啟動 | 技能觸發 | 800ms | 全屏邊框閃光 | `var(--color-action-secondary)` |
| 連擊提示 | 3 秒內 ≥3 擊殺 | 600ms | 「COMBO x{n}」衝屏文字 | `var(--color-action-primary)` |
...
| 卡片內容 | 武器 Sprite 128×128 px（置中上方）+ 武器名（text-h3）+ 倍率範圍（text-body，`var(--color-text-brand)`）+ 子彈速度（text-body-sm）+ 費用（text-body-sm，鑽石圖示）|
...
| 元素 | 規格 |
|------|------|
| 幣包商品 | 2 列，每格 320×200 px，商品卡規格；顯示金幣數量（大字 text-h1，`var(--color-text-brand)`）+ 美元價格（text-body）+ 加贈標籤（紅色 badge）|
| 暢銷標籤 | 右上角「🔥 最熱銷」badge，`var(--color-player-orange)` 底色 |
| 限時優惠 | 倒數計時條，`var(--color-feedback-error)` 底色，`var(--color-text-primary)` 文字 |
...
| 特權清單 | 清單項目：圖示（32 px）+ 說明文字（text-body）+ 狀態（已解鎖 `var(--color-feedback-success)` / 待解鎖 `var(--color-white-40)`）|
...
| 勝利狀態標題 | text-display，「恭喜！」`var(--color-text-brand)`，scale 0→1.2→1.0 彈入，500ms |
...
| Toggle 規格 | 52×32 px，開啟 `var(--color-feedback-success)`，關閉 rgba(255,255,255,0.3)（無精確 primitive） |
| 列表項目 | 高度 56 px，左側圖示 28 px + 文字 text-body，右側 Toggle 或箭頭 |
| 分隔線 | 1px solid rgba(255,255,255,0.1)（無精確 primitive，低於 color-border-subtle white-20） |
| 登出按鈕 | text-error `var(--color-text-error)`，無 Button 樣式，純文字點擊 |
...
| 元素 | 規格 |
|------|------|
| 頭部 | 全寬 banner 漸層（`var(--color-bg-surface)`→`var(--color-bg-base)`），高度 280 px |
| 頭像 | 96×96 px 圓形，邊框依 VIP 等級（§3.4 色系），Y=120 居中 |
| VIP 光暈 | 頭像外圈旋轉光暈（§3.4 規格） |
| 暱稱 | text-h2，`var(--color-text-primary)`，Y=230 |
| VIP 等級標籤 | 暱稱下方 badge，依等級顏色 |
| 統計卡片 | 三欄：總場次 / 最高倍率 / 累計獎金，每欄 215×100 px，card-bg |
| 成就系統 | 成就 Badge 網格（64×64 px，已解鎖全色，未解鎖灰色 30% opacity）|
| 交易記錄 | ScrollView 列表，每項 56 px 高，日期 + 金額（正值 `var(--color-feedback-success)`，負值 `var(--color-feedback-error)`）|
...
| 一般名次 | 列表行 64 px，左側名次數字（text-h3）+ 頭像 40 px + 暱稱 + VIP 徽章 + 右側獎金 `var(--color-text-brand)` |
| 玩家自身行 | 固定於列表底部，背景 rgba(245,200,66,0.1)（gold-400 @ 10%，無精確 primitive），border-top 1px `var(--color-border-brand)` |
...
| 進度點 | 5 個 dot，選中 `var(--color-action-primary)` 直徑 10 px，未選中 `var(--color-white-40)` 8 px |
| 下一步 CTA | Primary Button 240×48 px，「知道了」|
| 跳過按鈕 | 右上角，text-body-sm，`var(--color-text-secondary)` |
...
| Boss HP 中段（on hp bg）| #FFEB3B | #000000 | ~15.8:1 | AAA | ✓ |
| Boss HP 低（on hp bg） | #FF4444 | #000000 | 4.6:1 | AA | ✓ |
| VIP 金色文字（on bg-base）| #FFD700 | #051428 | ~13.3:1 | AAA | ✓ |
| VIP 金色文字（on bg-surface）| #FFD700 | #0A2340 | ~11.6:1 | AAA | ✓ |
...
.skeleton::after {
  ...
  background: linear-gradient(
    90deg,
    transparent 0%,
    rgba(13, 51, 96, 0.8) 50%, /* color-ocean-700 @ 80%；CSS gradient 不支援 var() 展開，直接值 */
    transparent 100%
  );
