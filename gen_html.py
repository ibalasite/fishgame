...
def md_to_html(text):
    ...
    while i < len(lines):
        ...
        if s.startswith('```mermaid'):
            ...
            out.append('<div class="diagram-container"><pre class="mermaid">' + mermaid_text + '</pre></div>')
        elif s.startswith('```'):
            lang = s[3:].strip()
            block = []
            i += 1
...
# UML diagram metadata — (label, icon, side)
# side: 'server' or 'frontend'
DIAGRAM_META = {
    ...
    'class-infra-presentation':  ('類別圖：基礎設施與呈現層',    '🏗️', 'server'),
    ...
    'sequence-login':            ('循序圖：登入',               '🔐', 'server'),
    'sequence-register':         ('循序圖：註冊',               '📝', 'server'),
    'sequence-shoot-fish':       ('循序圖：射擊魚群',           '🎯', 'server'),
    'sequence-shoot-fish-error': ('循序圖：射擊魚群（錯誤）',   '⚠️', 'server'),
    'sequence-iap-purchase':     ('循序圖：IAP 購買',           '💳', 'server'),
    'sequence-vip-subscribe':    ('循序圖：VIP 訂閱',           '⭐', 'server'),
    'communication':             ('協作圖',                    '🤝', 'server'),
    'state-machine-fish':        ('狀態機圖：魚群生命週期',      '🐟', 'server'),
    'state-machine-bullet':      ('狀態機圖：子彈生命週期',      '🔫', 'server'),
    'state-machine':             ('狀態機圖',                   '🔀', 'server'),
    'activity-flow':             ('活動圖：主流程',             '📊', 'server'),
    'activity-gameplay':         ('活動圖：遊戲流程',           '🎮', 'server'),
    'activity-purchase':         ('活動圖：購買流程',           '💳', 'server'),
    'activity-refund':           ('活動圖：退款流程',           '↩️', 'server'),
